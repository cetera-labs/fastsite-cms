...
Версия плагина — это git-тег вида `X.Y.Z` (например, `1.2.0`), без префикса `v`. Packagist берёт версии из тегов,
поэтому поле `version` в `composer.json` не указывается: иначе тег с другим номером будет отвергнут.

Новая версия выпускается тегом на основной ветке: исправления — `1.2.1`, новые возможности — `1.3.0`,
несовместимые изменения — `2.0.0`. Зависимости от CMS и других плагинов указываются нумерованными версиями
(`"cetera-labs/plugin-sale": "^1.0"`), а не `dev-master`.
