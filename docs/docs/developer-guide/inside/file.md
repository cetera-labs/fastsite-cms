...
Например, при активации темы (редакции) на конкретном сервере прописывается каталог *{% raw %}<DOCUMENT_ROOT>{% endraw %}/themes/<НАЗВАНИЕ ТЕМЫ>*, структура которого идентична .templates, за исключением того, что стили и js-скрипты (каталоги css и js) находятся также внутри каталога темы.

Замечание, касаемо сайтов, созданных в конструкторе fastsite.ru:

{% raw %}{% raw %}<DOCUMENT_ROOT>{% endraw %}{% endraw %} этих сайтов расположен в каталоге *www/subdmn/<ИМЯ САЙТА>*
