...
### Версии CMS и плагинов в composer.json

CMS и плагины подключаются в `composer.json` сайта только нумерованными версиями — так всегда видно, что стоит на проекте,
а `composer update` не приносит непроверенные изменения:

	"require": {
	    "cetera-labs/cetera-cms": "^3.80",
	    "cetera-labs/plugin-forms": "^1.0"
	}

Ветки (`dev-master`, `dev-main` и др.) не используются. Если сайту нужна доработка плагина, её вливают в основную ветку
плагина и выпускают новую версию. Что изменилось в CMS между версиями — в `Changelog.txt` пакета.
