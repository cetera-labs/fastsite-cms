...
Использование: dev/dev.sh <команда>
  ...
  phpstan  статический анализ ядра; накопленные замечания — в phpstan-baseline.neon
...
case $cmd in
    ...
    phpstan) php_exec php vendor/bin/phpstan analyse -c vendor/cetera-labs/cetera-cms/phpstan.neon.dist --memory-limit=1G "$@" ;;
