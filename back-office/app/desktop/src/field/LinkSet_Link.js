Ext.define('Cetera.field.LinkSet_Link', {
	...
	edit: function(id,type) {
        ...
        Ext.Loader.loadScript({
            url: '/cms/include/ui_material_edit.php?type='+type+'&idcat=-1&id='+id,
            onLoad: function() { 
    ...
    deleteMat: function() {
        Ext.MessageBox.confirm(Config.Lang.materialDelete, _('Вы уверены?'), function(btn) {
            if (btn == 'yes') {
                var rec = this.grid.getSelectionModel().getSelection()[0];
                Ext.Ajax.request({
    ...
    deleteLink: function() {
        Ext.MessageBox.confirm(_('Удалить связь материалов'), _('Вы уверены?'), function(btn) {
            if (btn == 'yes') {
                var rec = this.grid.getSelectionModel().getSelection()[0];
				Ext.Ajax.request({
					url: '/cms/include/action_materials.php',
