...
Выпуск — `sh dev/release.sh X.Y.Z`: меняет `VERSION`, дописывает в `Changelog.txt` раздел версии из заголовков коммитов
с прошлого тега, коммитит, ставит тег и пушит master с тегом (`--dry-run` — только показать раздел Changelog).
Поэтому заголовок коммита пишется как строка истории изменений: `CCTM-N Что изменилось`.
